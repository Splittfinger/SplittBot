...
import type { Agent, AgentAvatar, AgentInput, AppEvent, AppSnapshot, ChatImageAttachment, ConnectorInput, GuiEvidence, GuiSessionInput, GuiStep, Message, RoutineInput, Run, WorkspaceInput } from '../../shared/contracts'

type Section = 'home' | 'agents' | 'workspaces' | 'runs' | 'routines' | 'integrations' | 'computer' | 'approvals' | 'notifications' | 'artifacts' | 'memory' | 'audit' | 'settings'
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-pane">
        ...
        {section === 'approvals' ? <ApprovalsView snapshot={snapshot} onAction={runAction} /> : null}
      ...
      {agentEditor ? <AgentEditor value={agentEditor} agents={snapshot.agents} models={snapshot.models} connectors={snapshot.connectors} connectorAccounts={snapshot.connectorAccounts} skills={snapshot.skills} shortcuts={snapshot.shortcuts} fallbackCwd={selectedAgent?.cwd || '/Users'} onClose={() => setAgentEditor(null)} onSave={async (input) => {
        try {
...
function IntegrationsView({ snapshot, busy, onAction }: { snapshot: AppSnapshot; busy: boolean; onAction: (action: () => Promise<unknown>) => Promise<void> }): JSX.Element {
  ...
  async function saveConnectorAccount(): Promise<void> {
    await onAction(async () => {
      ...
      await window.splittbot.app.openExternal(result.authorizationUrl)
    })
  ...
  return <Page title="Tools & integrations" eyebrow="Reviewed capabilities" detail="Connect MCP services, review Codex skills, and approve structured Apple Shortcuts without storing API keys.">
    ...
    {tab === 'connectors' ? <>
      <section className="account-aware-panel panel">
...
function ApprovalsView({ snapshot, onAction }: { snapshot: AppSnapshot; onAction: (action: () => Promise<unknown>) => Promise<void> }): JSX.Element {
  return <Page title="Approvals" eyebrow="Human control" detail="Review what changes, what leaves the Mac, reversibility, and the exact post-approval behavior before continuing."><div className="approval-list">{snapshot.approvals.length ? snapshot.approvals.map((approval) => <ApprovalCard key={approval.id} approval={approval} agentName={snapshot.agents.find((agent) => agent.id === approval.agentId)?.name || 'Codex'} onAction={onAction} />) : <Empty icon={<ShieldCheck />} title="Nothing is waiting" detail="Approval requests from agents will appear here." />}</div></Page>
}
...
function AgentEditor({ value, agents, models, connectors, connectorAccounts, skills, shortcuts, fallbackCwd, onClose, onSave, onArchive }: {
  value: Agent | 'new'
  agents: Agent[]
  ...
  const [form, setForm] = useState<AgentInput>(() => existing ? {
    ...
    cwd: fallbackCwd,
    accessMode: 'readOnly',
    grants: { readableRoots: [fallbackCwd], writableRoots: [], allowedCommands: [], allowedApps: [], allowedConnectors: [], allowedConnectorAccounts: [], allowedSkillPaths: [], allowedShortcuts: [], networkAccess: false }
  })
  ...
  function toggleGrant(key: 'allowedConnectors' | 'allowedConnectorAccounts' | 'allowedSkillPaths' | 'allowedShortcuts', value: string): void {
