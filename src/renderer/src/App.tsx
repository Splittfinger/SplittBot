...
function RoutinesView({ snapshot, busy, onAction }: { snapshot: AppSnapshot; busy: boolean; onAction: (action: () => Promise<unknown>) => Promise<void> }): JSX.Element {
  ...
  async function create(): Promise<void> {
    const input: RoutineInput = {
      ...
      schedule: { ...(scheduleKind === 'interval' ? { kind: 'interval' as const, intervalMinutes } : { kind: 'daily' as const, timeOfDay, daysOfWeek }), ...(stopAfterDate ? { stopAfterDate } : {}) },
      catchUpPolicy, maxRetries, retryDelayMinutes: 5, notifyPolicy, skillPath: skillPath || null
    }
    await onAction(async () => {
      if (editingId) await window.splittbot.routines.update(editingId, input)
      else await window.splittbot.routines.create(input)
      setShowCreate(false)
      setEditingId(null)
    })
  }
  ...
  function edit(routine: AppSnapshot['routines'][number]): void {
    ...
    setStopAfterDate(routine.schedule.stopAfterDate ?? '')
  ...
  return <Page title="Routines" eyebrow="Awake-only scheduling" detail="SplittBot runs these while your Mac is awake and the app is open, with explicit catch-up and retry rules.">
    ...
    {showCreate ? <section className="inline-form panel">
      <div className="form-grid compact-form">
        ...
        <label className="wide"><span>Instructions</span><textarea aria-label="Routine instructions" value={prompt} onChange={(event) => setPrompt(event.target.value)} /></label>
        ...
        <label><span>Stop after date · optional</span><input aria-label="Routine stop date" type="date" value={stopAfterDate} onChange={(event) => setStopAfterDate(event.target.value)} /><small>Includes this date in your Mac’s local time zone. No runs or retries start afterward.</small></label>
...
function AgentEditor({ value, agents, models, connectedApps, connectors, connectorAccounts, skills, shortcuts, routines, fallbackCwd, onClose, onSave, onArchive }: {
  ...
  function newSchedule(): void {
    setSchedule({ id: null, title: `${form.name || 'Agent'} scheduled task`, prompt: 'Review the approved sources and complete this task. Do not send, publish, delete, or change permissions without a fresh approval.', scheduleKind: 'daily', intervalMinutes: 60, timeOfDay: '08:00', daysOfWeek: [1, 2, 3, 4, 5], stopAfterDate: '', catchUpPolicy: 'runOnce', notifyPolicy: 'always', maxRetries: 1, skillPath: '' })
  }
  ...
  function editSchedule(routine: AppSnapshot['routines'][number]): void {
    setSchedule({
      ...
      stopAfterDate: routine.schedule.stopAfterDate ?? '',
  ...
  function scheduleDraft(): AgentScheduleDraft | null {
    ...
    return {
      ...
      input: {
        ...
        schedule: { ...(schedule.scheduleKind === 'interval' ? { kind: 'interval' as const, intervalMinutes: schedule.intervalMinutes } : { kind: 'daily' as const, timeOfDay: schedule.timeOfDay, daysOfWeek: schedule.daysOfWeek }), ...(schedule.stopAfterDate ? { stopAfterDate: schedule.stopAfterDate } : {}) },
        ...
        <div className="wide agent-schedule-field">
          ...
          {schedule ? <div className="agent-schedule-form">
            <div className="form-grid compact-form">
              ...
              <label><span>Stop after date · optional</span><input aria-label="Schedule stop date" type="date" value={schedule.stopAfterDate} onChange={(event) => setSchedule({ ...schedule, stopAfterDate: event.target.value })} /><small>Inclusive, in your Mac’s local time zone.</small></label>
